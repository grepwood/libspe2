...
/* This test checks if DMA (not proxy DMA) and execution of stop
 * instruction are performed correctly at the same time. Essentially,
 * this is not for libspe test, however, this is useful for SPUFS
 * system test.
 */
...
extern spe_program_handle_t spu_dma_stop;
...
static void *spe_thread_proc(void *arg)
{
  ...
  for ( ; ; ) {
	...
	fatal();
      ...
      eprintf("Unexpected stop and signal (0x%08x).\n", ret);
      fatal();
    }
    else {
...
static int test(int argc, char **argv)
{
  ...
  generate_data(dma_data, 0, BUFFER_SIZE);
